...
void *create_shared_memory_area(unsigned int numbytes)
{
    ...
    /* TODO:
            addr = mmap(...)
    */
    addr = mmap(0, pages * sysconf(_SC_PAGE_SIZE), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    if (addr == MAP_FAILED)
    {
        printf("mmap  alloc error\n");
        exit(-1);
    }
    return addr;
}
...
int main(int argc, char *argv[])
{
    if (argc < 2)
    {
        printf("Input mode: Mode 0: Semaphores. Mode 1: Fork \n");
        return 1;
    }
    ...
    if (argc < 3)
    {
        printf("Input a number of threads/processes to use \n");
        return 1;
    }
    n = atoi(argv[2]);
    if (mode == 0)
    {
        s_list = (semstruct *)create_shared_memory_area(n * sizeof(semstruct)); // allocate mem for every thread we are going to use
        ...
        destroy_shared_memory_area(s_list, n * sizeof(semstruct));
    } // mode 0 = threads
    ...
    if (mode == 1)
    {
        ...
        int(*intarray)[x_chars] = (int(*)[x_chars])buffer;
        for (i = 0; i < n; i++)
        {
            p[i] = fork();
            if (p[i] < 0)
            ...
            {
                int t;
                for (t = i; t < y_chars; t += n)
                {
                    compute_mandel_line(t, intarray[t]);
                }
                exit(0);
            }
            // parent(p[i]);
        ...
        for (j = 0; j < y_chars; j++)
        {
            output_mandel_line(1, intarray[j]);
        }
